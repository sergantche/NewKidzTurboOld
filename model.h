...
public:			
	...
	class DummyTimeSpaceDistribution : public BoundaryCondition {
		double _pressure;
		double _temperature;
		Vector (*_velocityDistribution)( Vector, double );	//Velocity distribution
		MediumProperties medium;
		Model& mod;		//for total time
	public:

		class DummyTimeSpaceDistribution(Model& _model) : BoundaryCondition(_model), mod(_model) {			
			medium = model.medium;
		};

		void setParams(double pressure, double temperature, Vector (*vD)( Vector, double )) {
			_pressure = pressure;
			_temperature = temperature;
			_velocityDistribution = vD;
		};
		...
		ConservativeVariables getDummyValues(ConservativeVariables inV, const Face& face) {												
			double Pb = _pressure;
			double Tb = _temperature;
			Vector R_dummy = face.FaceCenter;	//consider dummy cell position to be same as face center one
			Vector vb = _velocityDistribution(R_dummy, mod.totalTime);			
			return model.PrimitiveToConservativeVariables(vb, Pb, Tb, medium);
		};
	};

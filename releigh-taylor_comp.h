...
bool RunFePuCollision1DTest(int N_cells, double time, double delta_time_to_save) {
	...
	model.SetBoundaryCondition("left", Symmetry);
...
//2D collision test with 500 m/s velocity of top (Pb) plate and Gausse acceleration
...
//Rihtmayer-Meshkov Instability
//2D initializing of init conditions
ConservativeVariables RihtmayerMeshkovInit2D(Vector CellPos, void* params) {
	Rectangle_Area_RT info = *(Rectangle_Area_RT*)params;
	...
	v.x = 0;
	v.z = 0;
	double shock_pos = 2.0*info.delta;
	//if(CellPos.y < shock_pos) v.y = info.A;
	//density
	double y_border = info.delta*sin((4*PI*CellPos.x - 10.5*3.14/58) /(info.x_max - info.x_min));		//density border
	if((CellPos.y <= y_border) && (CellPos.y >= info.y_min)) {
	double D = 600 ;
	double g = 1.67;
	double ro0 = 1.784 ;
	double p0 = 0.5 * 101325;
	double e0 = p0/(ro0*(g - 1));

	double u1=(D*D*(g-1)/g + 2*D*D + e0*0.67 - sqrt((D*D*(1-1/g)+ 2*D*D + e0*(g-1))*(D*D*(1-1/g)+ 2*D*D + e0*(g-1)) - 2*D*D*(g+1)*(2*e0*g-2*e0+D*D/2)/g))/(D*(g+1)/g) ;
	double e1 = e0+ D*D/(2*g)-(u1-D)*(u1-D)/(2*g) ;
	double ro1 = -ro0*D/(u1 - D) ; 
	double p1 = ro1*e1*(g-1);
		ro = info.ro_bot;
		p = p0/*info.P0*/;
		v.y = 0;
	};
	if((CellPos.y >= y_border) && (CellPos.y <= shock_pos)) {
	double D = 600 ;
	double g = 1.67;
	double ro0 = 1.784 ;
	double e0 = 0.5*101325/(1.784*0.67);
	double p0 = 0.5*101325 ;
	double u1=(D*D*(g-1)/g + 2*D*D + e0*0.67 - sqrt((D*D*(1-1/g)+ 2*D*D + e0*(g-1))*(D*D*(1-1/g)+ 2*D*D + e0*(g-1)) - 2*D*D*(g+1)*(2*e0*g-2*e0+D*D/2)/g))/(D*(g+1)/g) ;
	double e1 = e0+ D*D/(2*g)-(u1-D)*(u1-D)/(2*g) ;
	double ro1 = -ro0*D/(u1 - D) ; 
...
//	if ( CellPos.y <= shock_pos) {
	...
	if((CellPos.y >= shock_pos) && (CellPos.y <= info.y_max )) {
		double D = 600 ;
	double g = 1.67;
	double ro0 = 1.784 ;
	double e0 = 0.5*101325/(1.784*0.67);
...
//2D test for RMI
void RunRihtmayerMeshkov2D(int N_x, int N_y)
{
	//set task parameters
	Rectangle_Area_RT info;
	
	//Fe Pe discrete perturbation
	info.A = -854.318;		//velocity of collision for shock forming
	info.g = 0;		//no acceleration
	info.gamma = 1.67 /*1.4*/;
	info.P0 = 0.5*10121.9/*1.0e12*/;
	info.ro_bot = 5.894/*7.9e3*/;
	info.ro_top = 1.7839/*11.34e3*/;
	info.x_max = 0.036;
	info.x_min = -0.036;
	info.y_min = -0.09/*-0.03*/;
	info.y_max = 0.03;
	//amplitude of perturbation
	info.delta = 0.005 /*-0.4*info.y_min*/;

	double rhoTarget = 0.5 * (info.ro_bot + info.ro_top);
	...
	//Set initial conditions
	model.SetInitialConditions(RihtmayerMeshkovInit2D, &info);
	...
	bool isSaveSlice = true;
	//model.LoadSolution("solution_to_load.txt");

	//open amplitude history file and output header
	std::ofstream ofs("A.dat");
	ofs<<"VARIABLES = \"t\" \"A\"" << std::endl;
	...
	for (int i = 0; i < 1000000; i++) {
		...
		if (model.totalTime > /*3*/50.0e-5) break;

		//save amplitude
		double A = 0;
		
		ofs << model.totalTime << " " << A << std::endl;
		ofs.flush();

		//save solutions during computation
		if(model.totalTime > TimeToSave) {
			std::stringstream ss;
			ss << TimeToSave*1000000;
			std::string timer = ss.str();
			model.SaveToTechPlot(outputSolutionFile+timer+".dat");
			TimeToSave += delta_t;
		};
